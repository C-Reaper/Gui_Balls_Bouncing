...
void Update(AlxWindow* w){
	...
	if(dist > l){
		...
		if(ball_r<outer_r) 	ball_r = ball_r * 1.05f;
		else 				ball_r = outer_r;
	...
	Vec2 bg_r = TransformedView_WorldScreenLength(&tv,(Vec2){ outer_r,outer_r });
	...
	Vec2 r = TransformedView_WorldScreenLength(&tv,(Vec2){ ball_r,ball_r });
...
int main(){
    if(Create("Balls Bouncing",1900,1000,1,1,Setup,Update,Delete))
        Start();
    return 0;
}